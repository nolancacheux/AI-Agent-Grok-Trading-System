...
## CRITICAL: UPDATE .config/ FOLDER BEFORE EVERY COMMIT

**Before every commit, you MUST update the `.config/` folder:**
- `PROJECT.md` - Update if architecture, structure, or tech stack changes
- `INSTRUCTIONS.md` - Update if new rules or conventions are added

This keeps the documentation in sync with the codebase. Never commit without checking if these files need updates.
...
- Mentions of PROJECT.md or INSTRUCTIONS.md
...
6. [ ] **`.config/` folder updated** (PROJECT.md for architecture, INSTRUCTIONS.md for rules)
7. [ ] Commit message follows conventions

**IMPORTANT**: The `.config/` folder MUST be reviewed and updated before every commit to keep documentation in sync with code changes.
