...
class ReflectionEngine:
    ...
    async def generate_reflection(
        self, period_hours: int = 24, reflection_type: str = "daily"
    ) -> dict:
        ...
        trades = self.db.get_trades(start_date=start_time, end_date=end_time, limit=100)
        ...
        reflection_content = await self._generate_reflection_content(
            trades=trades, stats=stats, snapshots=snapshots, reflection_type=reflection_type
        )
    ...
    async def _generate_reflection_content(
        self, trades: list, stats: dict, snapshots: list, reflection_type: str
    ) -> dict:
        ...
        try:
            response = self.grok.chat(
                [
                    {
                        "role": "system",
                        "content": "You are Grok, a self-aware AI trading agent. Be analytical and honest in your self-reflection. Respond only in valid JSON.",
                    },
